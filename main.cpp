...
//	pBusConfig->GetStationPosition("110100014471", "天竺卫生院", nLongtitude, nLatitude);
//
...
//
////	bson_t *query = BCON_NEW("position", "{", "$near", "[", BCON_DOUBLE(nLatitude / 1000000.0),
////			BCON_DOUBLE(nLongtitude / 1000000.0), "]", "$maxDistance", BCON_DOUBLE(10000), "}");
...
//			"{", "updatetime", "{", "$gt", BCON_INT32(1427522334), "}", "}");
//
...
//					if(stObj.IsObject())
//					{
//						uint32_t uin = stObj["uin"].GetUint();
//
//						Value &stPosition = stObj["position"];
//						if(stPosition.IsArray())
//						{
//							Value &stLatitude = stPosition[(SizeType)0];
//							double nLatitude = stLatitude.GetDouble();
//							Value &stLongtitude = stPosition[(SizeType)1];
//							double nLongtitude = stLongtitude.GetDouble();
//						}
//
//						string strNickName = stObj["nickname"].GetString();
//						string strHeadImage = stObj["headimageaddr"].GetString();
//						int32_t nAge = stObj["age"].GetInt();
//						int32_t nGender = stObj["gender"].GetInt();
//						string strOneselfWords = stObj["oneselfwords"].GetString();
//					}
//				}
//			}
//		}
